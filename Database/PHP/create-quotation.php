...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $quo_no = mysqli_real_escape_string($link, $_POST['quo_no']); // *** ADDED THIS LINE ***
    ...
    try {
        ...
        // *** MODIFIED SQL QUERY TO INCLUDE `quo_no` ***
        $sql_insert_quotation = "INSERT INTO quotations (invoice_no, quote_date, customer_name, customer_address, customer_phone, delivery_address, terms, po_no, do_no, quo_no, attn, total_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql_insert_quotation)) {
            // *** MODIFIED BIND_PARAM: Added 's' for $quo_no and $quo_no variable ***
            mysqli_stmt_bind_param($stmt, "sssssssssssd", // 11 's' and 1 'd' = 12 total parameters
                $invoice_no,
                $quote_date,
                $customer_name,
                $customer_address,
                ...
                $po_no,
                $do_no,
                $quo_no, // Corresponding to the new 's' in type string and 'quo_no' in SQL
                $attn,
                $total_amount
            );
            mysqli_stmt_execute($stmt);
            $quotation_id = mysqli_insert_id($link); // Get the ID of the newly inserted quotation
            mysqli_stmt_close($stmt);
        } else {
            $success = false;
            throw new Exception("Error preparing quotation statement: " . mysqli_error($link));
        }
        ...
        if ($stmt_item = mysqli_prepare($link, $sql_insert_item)) {
            for ($i = 0; $i < count($item_descriptions); $i++) {
                ...
                // Ensure quantity parsing can handle "300 SET" by extracting just the number
                $numeric_qty = floatval(preg_replace('/[^0-9.]/', '', $qty));
                $item_total = ($price * $numeric_qty) - $discount;
                $calculated_total_amount += $item_total;

                mysqli_stmt_bind_param($stmt_item, "isssddd", $quotation_id, $item_code, $description, $qty, $price, $discount, $item_total);
                mysqli_stmt_execute($stmt_item);
            }
            mysqli_stmt_close($stmt_item);
        } else {
            $success = false;
            throw new Exception("Error preparing item statement: " . mysqli_error($link));
        }
        ...
        if ($stmt_update_total = mysqli_prepare($link, $sql_update_total)) {
            ...
            $success = false;
            throw new Exception("Error preparing total update statement: " . mysqli_error($link));
        }


        if ($success) {
            mysqli_commit($link);
            header("location: index.php"); // Redirect to index page on success
            exit();
        } else {
    ...
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>D.O No:</label>
                    <input type="text" name="do_no">
                </div>
                <div class="form-group">
                    <label>Quotation No:</label>
                    <input type="text" name="quo_no"> </div>
            </div>
            <div class="form-group">
                <label>ATTN:</label>
            ...
            <div class="button-group">
                <button type="submit" class="submit">Create Quotation</button>
    ...
    <script>
        ...
        function removeItem(button) {
            ...
            itemCounter = rows.length;
             if (itemCounter === 0) { // If all items are removed, add a default empty row
                addItem();
            }
        }

        // Initialize with at least one row if none exist on load (for edit.php usually)
        document.addEventListener('DOMContentLoaded', function() {
            const rows = document.getElementById('items-table').getElementsByTagName('tbody')[0].rows;
            if (rows.length === 0) {
                addItem(); // Add one initial item row if the page loads with no items
            }
        });
    </script>
